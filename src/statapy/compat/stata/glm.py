...
from statapy.compat.stata.factor_variables import expand_factor_terms
...
def logit(
    ...
    **kwargs,
    ...
    Maps to :class:`statapy.estimators.Logit`.
    """
    if kwargs:
...
def probit(
    ...
    **kwargs,
    ...
    Maps to :class:`statapy.estimators.Probit`.
    """
    if kwargs:
        raise ValueError(f"Unsupported arguments: {list(kwargs.keys())}")
...
def poisson(
    ...
    **kwargs,
    ...
    Maps to :class:`statapy.estimators.Poisson`.
    """
    if kwargs:
        raise ValueError(f"Unsupported arguments: {list(kwargs.keys())}")

    if exposure is not None:
        raise NotImplementedError("exposure is not yet supported in statapy.poisson")
    if offset is not None:
        raise NotImplementedError("offset is not yet supported in statapy.poisson")
