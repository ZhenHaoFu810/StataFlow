# Factor Variable Semantics in statapy

## Scope

This document records the Stata-compatible factor-variable syntax implemented in the `statapy.compat.stata` wrapper layer.
...
| `c.x1#c.x2` | continuous × continuous interaction | `c.x1#c.x2` |
| `c.x1##c.x2` | full factorial (main effects + interaction) | `x1`, `x2`, `c.x1#c.x2` |
| `i.g1#i.g2` | categorical × categorical interaction | `2.g1#2.g2`, `2.g1#3.g2`, ... |
| `i.g1##i.g2` | full factorial (main effects + interaction) | `2.g1`, `3.g1`, `2.g2`, `2.g1#2.g2`, ... |
| `i.g1#c.x1` | categorical × continuous interaction | `2.g1#c.x1`, `3.g1#c.x1`, ... |
...
| `x1#i.g` / `x1##i.g` | bare continuous mixed with categorical | 等价于 `c.x1#i.g` / `c.x1##i.g` |

## Naming Convention

Generated column names follow Stata coefficient naming as closely as possible:

- `i.g` with levels `[1, 2, 3]` → `2.g`, `3.g` (base level `1` omitted)
- `c.x1#c.x2` → `c.x1#c.x2`
- `i.g1#i.g2` → `<level1>.g1#<level2>.g2`
- `i.g1#c.x1` → `<level>.g1#c.x1`

## Rejected Syntax (ValueError)

The parser hard-rejects the following with a descriptive `ValueError`:

- `ib.<var>` — base indicators without a level number
- `b.<var>` — base levels without a level number
- `o.<var>` — omitted levels without a level number
- `L.x`, `F.x`, etc. — time-series operators
...
- `tests/golden/test_a2_factor_regress_basic.py` — `regress y i.g##c.x1`
- `tests/golden/test_a2_factor_regress_bare.py` — `regress y x1##x2` (Python) aligned with Stata `regress y c.x1##c.x2`
- `tests/golden/test_a2_factor_regress_base.py` — `regress y ib2.g##c.x1`
- `tests/golden/test_a2_factor_reghdfe_basic.py` — `reghdfe y i.g##c.x1, absorb(firm year)`
- `tests/golden/test_a2_factor_reghdfe_mixed.py` — `reghdfe y c.x1##i.g, absorb(firm year)` (mixed-order symmetry)
- `tests/golden/test_a2_factor_reghdfe_bare.py` — `reghdfe y x1##x2, absorb(firm year)` (Python) aligned with Stata `reghdfe y c.x1##c.x2, absorb(firm year)`
- `tests/golden/test_a2_factor_reghdfe_base.py` — `reghdfe y ib2.g##c.x1, absorb(firm year)`
- `tests/golden/test_a2_factor_ivreghdfe_basic.py` — `ivreghdfe y c.x1##i.g (x_endog = z1 z2), absorb(firm year)`
- `tests/golden/test_a2_factor_ppmlhdfe_basic.py` — `ppmlhdfe y i.g##c.x1, absorb(firm year)`
- `tests/golden/test_a2_factor_logit_basic.py` — `logit y_bin c.x1##c.x2`
- `tests/golden/test_a2_factor_logit_base.py` — `logit y_bin ib2.g##c.x1`
