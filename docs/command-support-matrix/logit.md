...
- Synthetic cases: Stata validation case `w3_logit_basic`
- Real-data cases: Stata validation case `w3_logit_real`
- Factor-syntax cases: Stata validation case `a2_factor_logit_basic` — `logit y_bin c.x1##c.x2`; Stata validation case `a2_factor_logit_base` — `logit y_bin ib2.g##c.x1`
- Margins cases: Stata validation case `w5_margins_logit_basic`, Stata validation case `w5_margins_real_mroz`
- Validated by Stata 17 comparison for MLE with conventional, robust, and cluster-robust VCE
