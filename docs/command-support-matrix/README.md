...
| [`rdrobust`](./rdrobust.md) | RD | Beta | Sharp / Fuzzy RD, 9 bandwidth selectors, covs, cluster/nncluster VCE, rdplot |
...
- Post-estimation (`predict`, `margins`) is available on the core estimator layer (`stataflow`), not directly on `compat.stata` wrappers. Some wrappers (e.g., `reghdfe`, `ppmlhdfe`) expose results through `ResultSchema` fields.
