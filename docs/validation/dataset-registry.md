...
| `jtrain` | DID | Wooldridge DID teaching data | `research/data/public/did/jtrain_prepared.dta` | Prepared local mirror from open teaching data | `did_imputation`, `eventstudyinteract`, `csdid` | `research/data/public/did/prepare_jtrain.py` | OOS validated (1 blocked) |
...
| `rdrobust_senate` | RD | Official `rdrobust` Stata package | `research/vendor/stata_community/rdrobust/rdrobust-master/stata/rdrobust_senate.dta` | Vendor research mirror | `rdrobust` | `stata/cases/rdrobust_gen_z.do` | validated (development + OOS) |
...
- `jtrain` 已作为 OOS DID 验证数据集使用（`eventstudyinteract`、`csdid` 通过；`did_imputation` 因短面板限制 blocked）。
