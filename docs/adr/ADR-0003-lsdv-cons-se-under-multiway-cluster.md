...
MAP 吸收内核已经可用，但该 `_cons` 恢复路径的差异仍需按本 ADR 管理：
...
- 优点：彻底消除差异
- 缺点：重大架构变更，需要重写相关 HDFE 协方差与常数恢复路径
...
- 每个受影响的 Stata 验证用例必须引用 ADR-0003
...
- Stata validation case `w7_reghdfe_2way_cluster` L200-210（rtol=0.03）
- Stata validation case `w7_reghdfe_2way_cluster_real` L170-183（rtol=0.20）
- Stata validation case `w7_ivreghdfe_2way_cluster` L197-207（rtol=0.03）
- Stata validation case `w7_ppmlhdfe_2way_cluster` L166-176（rtol=0.03）
- `docs/command-support-matrix/reghdfe.md`、`ivreghdfe.md`、`ppmlhdfe.md`

## 何时重审

- `_cons` 恢复或协方差构造发生重大变化
- LSDV T 矩阵构造方式发生重大变化
...
因此，`_cons` SE 的 2-way cluster 残差继续按本 ADR 的容忍度和披露要求管理。
