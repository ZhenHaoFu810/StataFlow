"""
Golden test: p1_ols_basic - Basic OLS regression test.

Tests basic OLS functionality with constant term, verifying:
...
def _generate_test_data() -> pd.DataFrame:
    ...
    # True model: y = 10 + 3*x1 - 2*x2 + error
    y = 10 + 3 * x1 - 2 * x2 + np.random.normal(0, 1.5, n)

    return pd.DataFrame({"y": y, "x1": x1, "x2": x2})


def _run_stata_basic(data: pd.DataFrame) -> dict:
    """Run Stata regress for basic test."""
    dta_file = PROJECT_STATA_CASES / "p1_ols_basic_data.dta"
...
class TestP1OlsBasic:
    """Golden test for p1_ols_basic."""
    ...
    def stata_result(self, test_data):
        """Get Stata results."""
        return _run_stata_basic(test_data)
