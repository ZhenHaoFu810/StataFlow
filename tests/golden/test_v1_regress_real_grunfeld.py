"""
Golden test: v1_regress_real_grunfeld - OLS on Grunfeld investment panel.

Real-data validation using the classic Grunfeld panel as a plain OLS benchmark.
"""
...
def _parse_stata_log(log_content: str) -> dict:
    ...
    for name in ["value", "capital", "_cons"]:
...
def _run_stata_regress(data: pd.DataFrame) -> dict:
    dta_file = PROJECT_STATA_CASES / "v1_regress_real_grunfeld_data.dta"
...
class TestV1RegressRealGrunfeld:
    ...
    def python_result(self, test_data):
        return regress(test_data, y="inv", x=["value", "capital"], vce="ols")

    @pytest.fixture(scope="class")
    def stata_result(self, test_data):
        return _run_stata_regress(test_data)
    ...
    def test_r2(self, python_result, stata_result):
        passed, msg = tolerance_close(python_result.fit.r2, stata_result["r2"], name="r2")
        assert passed, msg

    def test_r2_adj(self, python_result, stata_result):
        passed, msg = tolerance_close(python_result.fit.r2_adj, stata_result["r2_adj"], name="r2_adj")
        assert passed, msg
