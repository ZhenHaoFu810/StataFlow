"""
Golden test: w10_liml_weak - LIML estimator.

Tests ivreghdfe-style LIML estimator with one absorbed FE,
...
def _run_stata(data: pd.DataFrame) -> dict:
    """Run Stata ivreghdfe with liml."""
    dta_file = PROJECT_STATA_CASES / "w10_liml_weak_data.dta"
...
ivreghdfe y x1 (x2 = z1 z2), absorb(entity_id) keepsingletons liml
...
def _run_python(data: pd.DataFrame):
    """Run Python IVAbsorbingOLS with liml."""
    ...
    return model.fit(vce="ols", estimator="liml")


class TestW10LimlWeak:
    """Golden test for w10_liml_weak."""
