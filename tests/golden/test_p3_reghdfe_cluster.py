"""
Golden test: p3_reghdfe_cluster - reghdfe with 2 FEs and cluster-robust SE (synthetic).

Tests that Python AbsorbingOLS with absorb=[var1, var2] and vce='cluster'
matches Stata's reghdfe:
- Coefficient estimates
- Cluster-robust standard errors
- Cluster count
- F-statistic (Wald test with cluster df)
- Degrees of freedom (including df_a with cluster nesting adjustment)
"""
...
def _run_stata_reghdfe_cluster(data: pd.DataFrame) -> dict:
    """Run Stata reghdfe with 2 FEs and cluster-robust SE."""
    dta_file = PROJECT_STATA_CASES / "p3_reghdfe_cluster_data.dta"
...
display "Stata reghdfe cluster completed successfully"
'''
    runner = StataRunner()
...
class TestP3ReghdfeCluster:
    """Golden test for p3_reghdfe_cluster."""
    ...
    def stata_result(self, test_data):
        """Get Stata reghdfe + cluster results."""
        return _run_stata_reghdfe_cluster(test_data)
