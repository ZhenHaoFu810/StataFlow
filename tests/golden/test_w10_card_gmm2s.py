"""
Golden test: w10_card_gmm2s - GMM2S on Card returns-to-schooling data.

Tests that Python IVAbsorbingOLS with estimator='gmm2s' matches Stata's
ivreghdfe gmm2s on real data.
"""
...
def _run_stata() -> dict:
    """Run Stata ivreghdfe with gmm2s on Card data."""
    do_template = f'''
clear all
set more off
...
ivreghdfe lwage exper expersq black smsa reg661-reg668 smsa66 (educ = nearc4), absorb(south) keepsingletons gmm2s
...
def _run_python(data: pd.DataFrame):
    """Run Python ivreghdfe with gmm2s on Card data."""
...
class TestW10CardGmm2s:
    """Golden test for w10_card_gmm2s."""
