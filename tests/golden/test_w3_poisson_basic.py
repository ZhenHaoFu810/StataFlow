"""
Golden test: w3_poisson_basic - Basic poisson regression test.

Tests basic poisson functionality with constant term, verifying:
...
def _generate_test_data() -> pd.DataFrame:
    """Generate basic poisson test dataset with known seed."""
...
def _run_stata_poisson(data: pd.DataFrame) -> dict:
    """Run Stata poisson for basic test."""
    dta_file = PROJECT_STATA_CASES / "w3_poisson_basic_data.dta"
...
display "Stata poisson completed successfully"
'''
    runner = StataRunner()
...
class TestW3PoissonBasic:
    """Golden test for w3_poisson_basic."""
    ...
    def python_result(self, test_data):
        model = Poisson(data=test_data, y="y", x=["x1", "x2"], add_constant=True)
