"""
Golden test: w9_di_controls_basic - did_imputation with controls.

Tests did_imputation with controls(x1 x2) on synthetic staggered adoption data,
verifying event time coefficients (tauh) and standard errors align with Stata 17.
"""
...
def _generate_test_data() -> pd.DataFrame:
    ...
    # Control variables (vary by unit and time)
    x1 = np.random.normal(0, 1, len(ids))
    x2 = np.random.normal(0, 1, len(ids))

    # Outcome with control effects
    y = fe_i + fe_t + 0.5 * x1 - 0.3 * x2 + 1.5 * treat + np.random.normal(0, 1, len(ids))
...
def _run_stata_did_imputation(data: pd.DataFrame) -> dict:
    """Run Stata did_imputation with controls."""
    dta_file = PROJECT_STATA_CASES / "w9_di_controls_basic_data.dta"
...
did_imputation y id year first_treat, controls(x1 x2) allhorizons cluster(id) autosample
...
class TestW9DiControlsBasic:
    """Golden test for w9_di_controls_basic."""
