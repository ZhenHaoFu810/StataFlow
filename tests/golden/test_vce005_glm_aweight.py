"""VCE-005 golden test: weighted GLM robust/cluster sandwich weight order.

Stata `glm [aweight=w], family(poisson) link(log) vce(robust/cluster)` uses
score = w * (y - mu) * x (Poisson log-link is canonical). This test verifies
Python matches Stata after the VCE-005 fix switched from sqrt(w)*x*e to w*x*e.
"""
...
def _generate_data() -> pd.DataFrame:
    ...
    eta = 0.5 + 0.3 * x1 - 0.4 * x2
    mu = np.exp(eta)
    y = np.random.poisson(mu)
    weights = np.abs(np.random.exponential(scale=1.5, size=n)) + 0.5
    group = np.random.randint(0, 15, size=n)
    return pd.DataFrame({"y": y, "x1": x1, "x2": x2, "w": weights, "g": group})


def _run_stata(data: pd.DataFrame, vce: str) -> dict:
    dta_file = PROJECT_STATA_CASES / f"vce005_poisson_{vce}_data.dta"
    data.to_stata(str(dta_file), write_index=False)
    cmd = f"glm y x1 x2 [aweight=w], family(poisson) link(log) vce({vce})"
    do_template = f'''
clear all
set more off
use "{dta_file}", clear
...
display "VCE005_POISSON_{vce.upper()} completed"
'''
    runner = StataRunner()
...
class TestVCE005GLMAweightRobust:
    ...
    def python_result(self, data):
        return poisson(data, y="y", x=["x1", "x2"], aweight="w", vce="robust")
...
class TestVCE005GLMAweightCluster:
    @pytest.fixture(scope="class")
    def data(self):
        return _generate_data()
    ...
    @pytest.fixture(scope="class")
    def python_result(self, data):
        return poisson(data, y="y", x=["x1", "x2"], aweight="w", vce="cluster", cluster="g")
