"""
Golden test: a2_factor_reghdfe_base

Tests Stata factor-variable base-level syntax alignment for ``reghdfe`` with
a categorical-continuous full interaction ``ib2.g##c.x1`` and two
...
display "COEF 1.g " _b[1.g] " " _se[1.g]
display "COEF 3.g " _b[3.g] " " _se[3.g]
display "COEF x1 " _b[x1] " " _se[x1]
display "COEF 1.g#c.x1 " _b[1.g#c.x1] " " _se[1.g#c.x1]
...
class TestA2FactorReghdfeBase:
    ...
    def python_result(self, test_data):
        return reghdfe(test_data, y="y", x=["ib2.g##c.x1"], absorb="firm year")
    ...
    def test_coefficients_beta(self, python_result, stata_result):
        for py_coef, st_coef in zip(
            ...
            rtol = 1e-2 if py_coef.name == "_cons" else 1e-6
            passed, msg = tolerance_close(
