"""
Golden test: w12_slopes_zero - slope absorption with all-zero group (synthetic).

Tests that Python reghdfe handles a group where the slope variable is all zero.
In this case the within-group regression is singular; reghdfe drops the slope
for that group (it is collinear with the intercept).
"""
...
def _generate_test_data() -> pd.DataFrame:
    """Generate test dataset where firm 0 has time=0 for all observations."""
    ...
    # Force time = 0 for firm 0, making slope singular within that group
    time[firm_id == 0] = 0.0
...
def _run_stata(data: pd.DataFrame) -> dict:
    """Run Stata reghdfe with slope absorption on zero-slope group."""
    dta_file = PROJECT_STATA_CASES / "w12_slopes_zero_data.dta"
...
reghdfe y x, absorb(firm_id##c.time) keepsingletons
...
display "Stata slopes zero completed successfully"
'''
    runner = StataRunner()
