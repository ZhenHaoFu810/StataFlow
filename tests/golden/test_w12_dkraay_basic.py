"""
Golden test: w12_dkraay_basic - Driscoll-Kraay HAC VCE (synthetic).

Tests that Python reghdfe with vce(dkraay) matches Stata.
"""
...
def _run_stata(data: pd.DataFrame) -> dict:
    """Run Stata reghdfe with Driscoll-Kraay VCE."""
    dta_file = PROJECT_STATA_CASES / "w12_dkraay_basic_data.dta"
...
display "Stata dkraay basic completed successfully"
'''
    runner = StataRunner()
...
class TestW12DkraayBasic:
    ...
    def test_coefficients_std_err(self, python_result, stata_result):
        for py_coef, st_coef in zip(
            ...
            # DK SE tolerance relaxed to 1e-4 per instructions
            passed, msg = tolerance_close(
