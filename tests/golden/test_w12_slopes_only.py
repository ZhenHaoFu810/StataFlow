"""
Golden test: w12_slopes_only - pure slope absorption (synthetic).

Tests that Python reghdfe with absorb(firm_id#c.time) matches Stata.
Note: #c. (not ##c.) means slope-only, no intercept per firm.
"""
...
def _run_stata(data: pd.DataFrame) -> dict:
    """Run Stata reghdfe with pure slope absorption."""
    dta_file = PROJECT_STATA_CASES / "w12_slopes_only_data.dta"
...
display "Stata slopes only completed successfully"
'''
    runner = StataRunner()
