"""
P1 verification: Poisson robust + cluster VCE correction factors.

Validates VCE-P1-3b: does Stata poisson apply HC1 correction for robust VCE?
...
def _run_stata_robust(data: pd.DataFrame) -> dict:
    dta_file = PROJECT_STATA_CASES / "p1v_poisson_robust_data.dta"
...
display "P1V_POISSON_ROBUST completed"
'''
    runner = StataRunner()
...
def _run_stata_cluster(data: pd.DataFrame) -> dict:
    dta_file = PROJECT_STATA_CASES / "p1v_poisson_cluster_data.dta"
    data.to_stata(str(dta_file), write_index=False)
    do_template = f'''
clear all
set more off
use "{dta_file}", clear
poisson y x1 x2, vce(cluster group)
display "E_N=" e(N)
display "B_X1=" _b[x1]
display "B_X2=" _b[x2]
display "SE_X1=" _se[x1]
display "SE_X2=" _se[x2]
display "P1V_POISSON_CLUSTER completed"
'''
    runner = StataRunner()
    result = runner.run_do_file(do_template, output_dir=str(PROJECT_STATA_OUTPUT))
    if result.exit_code != 0:
...
class TestP1VPoissonRobust:
    ...
    def python_result(self, test_data):
        model = Poisson(data=test_data, y="y", x=["x1", "x2"])
