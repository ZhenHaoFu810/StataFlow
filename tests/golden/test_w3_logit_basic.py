"""
Golden test: w3_logit_basic - Basic logit regression test.

Tests basic logit functionality with constant term, verifying:
...
def _generate_test_data() -> pd.DataFrame:
    """Generate basic logit test dataset with known seed."""
...
def _run_stata_logit(data: pd.DataFrame) -> dict:
    """Run Stata logit for basic test."""
    dta_file = PROJECT_STATA_CASES / "w3_logit_basic_data.dta"
...
display "Stata logit completed successfully"
'''
    runner = StataRunner()
...
class TestW3LogitBasic:
    """Golden test for w3_logit_basic."""
    ...
    def python_result(self, test_data):
        model = Logit(data=test_data, y="y", x=["x1", "x2"], add_constant=True)
