"""
Golden test: w10_kclass_basic - LIML with user-specified k-class(0.5).

Tests ivreghdfe-style k-class estimator with one absorbed FE,
...
def _run_stata(data: pd.DataFrame) -> dict:
    """Run Stata ivreghdfe with kclass(0.5)."""
    dta_file = PROJECT_STATA_CASES / "w10_kclass_basic_data.dta"
...
ivreghdfe y x1 (x2 = z1 z2), absorb(entity_id) keepsingletons kclass(0.5)
...
def _run_python(data: pd.DataFrame):
    """Run Python IVAbsorbingOLS with kclass(0.5)."""
    ...
    return model.fit(vce="ols", estimator="liml", kclass=0.5)


class TestW10KclassBasic:
    """Golden test for w10_kclass_basic."""
