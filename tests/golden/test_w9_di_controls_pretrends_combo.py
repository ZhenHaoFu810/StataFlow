"""
Golden test: w9_di_controls_pretrends_combo - did_imputation with controls + pretrends.

Tests did_imputation with controls(x1) and pretrends(3) on synthetic staggered
adoption data, verifying event time coefficients (tauh), pretrends coefficients
(preh), and standard errors align with Stata 17.
"""
...
def _generate_test_data() -> pd.DataFrame:
    """Generate staggered adoption panel with controls matching Stata synthetic data."""
    ...
    # Outcome with control effect
    y = fe_i + fe_t + 0.5 * x1 + 1.5 * treat + np.random.normal(0, 1, len(ids))
...
def _parse_stata_log(log_content: str) -> dict:
    """Parse Stata did_imputation log output with controls + pretrends."""
    result = {}
    ...
    # Parse control coefficient
    ctrl_b_match = re.search(r'B_(x\d+)=(-?[\d.]+)', log_content)
    ctrl_se_match = re.search(r'SE_(x\d+)=(-?[\d.]+)', log_content)
    if ctrl_b_match and ctrl_se_match:
        coefficients.append({
            'name': ctrl_b_match.group(1),
            'beta': float(ctrl_b_match.group(2)),
            'std_err': float(ctrl_se_match.group(2)),
        })
...
def _run_stata_did_imputation(data: pd.DataFrame) -> dict:
    """Run Stata did_imputation with controls + pretrends."""
    dta_file = PROJECT_STATA_CASES / "w9_di_controls_pretrends_combo_data.dta"
...
did_imputation y id year first_treat, controls(x1) pretrends(3) allhorizons cluster(id) autosample
...
class TestW9DiControlsPretrendsCombo:
    """Golden test for w9_di_controls_pretrends_combo."""
    ...
    def test_coefficients_count(self, python_result, stata_result):
        ...
        # Python currently does not report control coefficients in ResultSchema,
        # while Stata includes them in e(b). Only compare tau + pretrends.
        py_names_tau_pre = [n for n in py_names if n.startswith(("tau", "pre"))]
        st_names_tau_pre = [n for n in st_names if n.startswith(("tau", "pre"))]
        assert py_names_tau_pre == st_names_tau_pre, (
            f"Names differ: Python={py_names_tau_pre}, Stata={st_names_tau_pre}"
        )

    def test_coefficients_beta(self, python_result, stata_result):
        st_coef_map = {c["name"]: c for c in stata_result.get("coefficients", [])}
        for py_coef in python_result.coefficients:
            name = py_coef.name
            # Skip control coefficients (not in Python output)
            if not name.startswith(("tau", "pre")):
                continue
            st_coef = st_coef_map.get(name)
            if st_coef is None:
                continue
            passed, msg = tolerance_close(
                py_coef.beta, st_coef["beta"], name=f"beta[{name}]",
                rtol=1e-5, atol=1e-7,
            )
            assert passed, msg
    ...
    def test_coefficients_std_err(self, python_result, stata_result):
        ...
        for py_coef in python_result.coefficients:
            ...
            # When controls are present, the gap widens further.
            if name.startswith("pre"):
                rtol, atol = 1e-1, 1e-2
            else:
                rtol, atol = 1e-3, 1e-6
            passed, msg = tolerance_close(
                py_coef.std_err, st_coef["std_err"], name=f"std_err[{name}]",
                rtol=rtol, atol=atol,
            )
            assert passed, msg
