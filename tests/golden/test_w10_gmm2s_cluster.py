"""
Golden test: w10_gmm2s_cluster - GMM2S with cluster-robust VCE.

Tests ivreghdfe-style GMM2S estimator with cluster VCE,
verifying coefficients, SEs, and Hansen J statistic.
"""
...
def _run_stata(data: pd.DataFrame) -> dict:
    """Run Stata ivreghdfe with gmm2s and cluster VCE."""
    dta_file = PROJECT_STATA_CASES / "w10_gmm2s_cluster_data.dta"
...
ivreghdfe y x1 (x2 = z1 z2), absorb(entity_id) keepsingletons vce(cluster entity_id) gmm2s
...
def _run_python(data: pd.DataFrame):
    """Run Python IVAbsorbingOLS with gmm2s and cluster VCE."""
    ...
    return model.fit(vce="cluster", cluster="entity_id", estimator="gmm2s")


class TestW10Gmm2sCluster:
    """Golden test for w10_gmm2s_cluster."""
    ...
    def test_cluster_count(self, python_result, stata_result):
        passed, msg = tolerance_close(
            python_result.diagnostics.cluster_count, stata_result.get("n_clust"), name="n_clust"
        )
        assert passed, msg
    ...
    def test_hansen_j(self, python_result, stata_result):
        ...
        assert py_j is not None
        assert st_j is not None
