"""
Golden test: w2_ivregress_cluster - 2SLS with cluster-robust VCE.

Tests ivregress 2sls with vce(cluster), verifying:
...
def _generate_test_data() -> pd.DataFrame:
    """Generate clustered IV test dataset with known seed."""
    ...
    # Entity-level shock correlated with x2 and u
    entity_shock = np.repeat(np.random.normal(0, 1, n_entities), n_per_entity)
    v = np.random.normal(0, 0.5, n) + entity_shock
    x2 = 0.5 + 1.5 * z1 - 0.8 * z2 + 0.3 * x1 + v

    u = np.random.normal(0, 0.5, n) + 0.5 * entity_shock
    y = 3 + 2 * x1 + 1.5 * x2 + u
...
def _run_stata_ivregress_cluster(data: pd.DataFrame) -> dict:
    """Run Stata ivregress 2sls with cluster VCE."""
    dta_file = PROJECT_STATA_CASES / "w2_ivregress_cluster_data.dta"
...
// Run 2SLS with cluster-robust VCE
ivregress 2sls y x1 (x2 = z1 z2), vce(cluster entity_id)
...
display "Stata ivregress 2sls cluster completed successfully"
'''
    do_content = do_template.replace("$DATA_FILE", str(dta_file))
    return run_stata_ivregress(do_content, coef_names=["x2", "x1", "_cons"])


class TestW2IvregressCluster:
    """Golden test for w2_ivregress_cluster."""
    ...
    def stata_result(self, test_data):
        return _run_stata_ivregress_cluster(test_data)
