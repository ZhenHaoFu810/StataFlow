"""Shared utilities for M03 HDFE modular revalidation v1.3.
...
STATA_CASES = PROJECT_ROOT / "stata" / "cases" / "audit_v1_3_m03"
STATA_OUTPUT = PROJECT_ROOT / "stata" / "output" / "audit_v1_3_m03"
EVIDENCE = PROJECT_ROOT / "docs" / "audit" / "modular-revalidation-v1.3" / "M03-hdfe" / "evidence"
...
def python_result_to_dict(result) -> dict[str, Any]:
    ...
    # Attach model-internal fields if available.
...
def compare_coefficients(py_coefs: list[dict], st_coefs: list[dict], rtol: float = 1e-6, atol: float = 1e-8):
    ...
    """
    results = []
    py_names = [c["name"] for c in py_coefs]
