"""Shared utilities for M04 IV/GMM modular revalidation v1.3."""
...
STATA_CASES = PROJECT_ROOT / "stata" / "cases" / "audit_v1_3_m04"
STATA_OUTPUT = PROJECT_ROOT / "stata" / "output" / "audit_v1_3_m04"
EVIDENCE = PROJECT_ROOT / "docs" / "audit" / "modular-revalidation-v1.3" / "M04-iv-gmm" / "evidence"
...
def parse_stata_log(log_content: str) -> dict[str, Any]:
    ...
    e_patterns = {
        ...
        "widstat": r"E_WIDSTAT=([\d.eE+-]+)",
        "idstat": r"E_IDSTAT=([\d.eE+-]+)",
        "j_stat": r"E_J=([\d.eE+-]+)",
        "j_pvalue": r"E_J_P=([\d.eE+-]+)",
        "j_df": r"E_J_DF=([\d]+)",
        "df_a": r"E_DF_A=([\d.eE+-]+)",
        "num_singletons": r"E_N_SINGLETONS=([\d]+)",
    }
...
def python_result_to_dict(result) -> dict[str, Any]:
    ...
    # Attach IV diagnostics if present
    for attr in ["widstat", "idstat", "hansen_j", "hansen_j_df", "hansen_j_pvalue"]:
        val = getattr(result.diagnostics, attr, None)
        if val is not None:
            out[attr] = val
    model = getattr(result, "_model", None)
    if model is not None:
        if hasattr(model, "_df_a"):
            out["df_a"] = float(model._df_a)
        if hasattr(model, "_num_singletons"):
            out["num_singletons"] = int(model._num_singletons)
