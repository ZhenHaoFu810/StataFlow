...
STATA_CASES = PROJECT_ROOT / "stata" / "cases" / "audit_v1_3_m02"
STATA_OUTPUT = PROJECT_ROOT / "stata" / "output" / "audit_v1_3_m02"
EVIDENCE = PROJECT_ROOT / "docs" / "audit" / "modular-revalidation-v1.3" / "M02-panel-fe" / "evidence"
...
def save_evidence(test_id: str, py_dict: dict, st_dict: Optional[dict], comparisons: list[tuple[bool, str]], data: Optional[pd.DataFrame] = None):
    ...
    ev_dir = EVIDENCE / "synthetic" if test_id.startswith("S") else EVIDENCE / "real-data" if test_id.startswith("R") else EVIDENCE / "property" if test_id.startswith("P") else EVIDENCE / "minimal-reproductions"
