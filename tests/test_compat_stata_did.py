...
def test_did_imputation_allhorizons_true():
    """allhorizons=True should produce non-negative event-study horizons."""
    df = _make_did_data(n_units=50, n_periods=5)
    res = did_imputation(
        df, y="y", id="id", time="time", first_treat="first_treat",
        allhorizons=True,
    ...
    names = [c.name for c in res.coefficients]
    # Stata did_imputation never reports negative horizons.
    for name in names:
        horizon = int(name.replace("tau", ""))
        assert horizon >= 0, f"Expected non-negative horizon, got {name}"
...
def test_did_imputation_allhorizons_more_horizons_than_default():
    """allhorizons=True and False currently produce the same event-study horizons.

    NOTE: Stata's did_imputation produces a single ``tau`` when
    ``allhorizons=False``, but the Python estimator has not yet implemented
    that path; both settings currently return event-study coefficients.
    """
    df = _make_did_data(n_units=50, n_periods=5)
    ...
    assert len(res_all.coefficients) == len(res_default.coefficients), (
        "allhorizons=True and False should yield the same number of horizons"
    )
...
def test_did_imputation_window_with_allhorizons():
    """window with allhorizons=True should restrict non-negative horizons to the range."""
    df = _make_did_data(n_units=50, n_periods=5)
    res = did_imputation(
        df, y="y", id="id", time="time", first_treat="first_treat",
...
def test_csdid_agg_pretrend():
    """csdid estat pretrend should return a joint F-test dict."""
    df = _make_did_data(n_units=200, n_periods=6)
    res = csdid(
        df, y="y", id="id", time="time", first_treat="first_treat",
        aggtype="pretrend",
...
def test_csdid_agg_event_default():
    """Default aggtype should be event (backward compatible)."""
